...
export default function ProtocolCard({ protocol, workspaceId }: ProtocolCardProps) {
  ...
  return (
    <Link href={`/dashboard/guidelines/protocols/${protocol.id}`}>
      <div className="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow p-6 cursor-pointer group">
        ...
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1">
            {protocol.category && (
              ...
              >
                <span>{protocol.category.icon}</span>
                <span>{protocol.category.name}</span>
              </div>
            )}
        ...
        {protocol.description && (
          <p className="text-sm text-gray-600 line-clamp-2 mb-4">{protocol.description}</p>
        )}
        ...
        <div className="flex items-center gap-4 text-sm text-gray-500 pt-4 border-t border-gray-100">
          <div className="flex items-center gap-1">
            ...
            <span>{protocol.view_count}</span>
          </div>
          <div className="flex items-center gap-1">
            <Star className="h-4 w-4" />
            <span>{protocol.favorite_count}</span>
          </div>
          <div className="flex items-center gap-1 ml-auto">
            <Clock className="h-4 w-4" />
            <span>{new Date(protocol.updated_at).toLocaleDateString('tr-TR')}</span>
          </div>
        </div>
      </div>
    </Link>
