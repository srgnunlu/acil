...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json(mentionsWithUsers)
  } catch (error) {
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (user_ids.length > 10) {
      return NextResponse.json({ error: 'Maximum 10 mentions allowed' }, { status: 400 })
    }
    ...
    return NextResponse.json({ mentions: mentionsWithUsers }, { status: 201 })
  } catch (error) {
